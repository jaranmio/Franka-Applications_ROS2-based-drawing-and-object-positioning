...
const std::string IMAGE_PATH = "/home/qpaig/my_ros2_ws/src/fr3_generic_drawing/src/franka_sketch4.jpg";
...
void dfsTrace(const cv::Mat& skeleton, cv::Point pt, std::vector<cv::Point>& contour, cv::Mat& visited) {
    ...
    for (int k = 0; k < 8; ++k) {
        ...
        if (np.x < 0 || np.y < 0 || np.x >= skeleton.cols || np.y >= skeleton.rows)
            continue;
        if (skeleton.at<uchar>(np) == 255 && visited.at<uchar>(np) == 0) {
            dfsTrace(skeleton, np, contour, visited);
            return;  // Simple trace (no branching)
        }
    }
}

// Extract all centerline paths from a skeletonized image
std::vector<std::vector<cv::Point>> extractPathsFromSkeleton(const cv::Mat& skeleton) {
    std::vector<std::vector<cv::Point>> contours;
    cv::Mat visited = cv::Mat::zeros(skeleton.size(), CV_8UC1);

    for (int y = 0; y < skeleton.rows; ++y) {
        for (int x = 0; x < skeleton.cols; ++x) {
            if (skeleton.at<uchar>(y, x) == 255 && visited.at<uchar>(y, x) == 0) {
                std::vector<cv::Point> contour;
                dfsTrace(skeleton, cv::Point(x, y), contour, visited);
                if (contour.size() > 5) {
...
int main(int argc, char** argv) {
    ...
    std::vector<std::vector<cv::Point>> contours = extractPathsFromSkeleton(binary);
    contours = mergeCloseContours(contours, 5.0);
    auto smoothContours2f = smoothAndResampleContours(contours, 1.5);

    // Helper code to see extracted contours
    std::vector<std::vector<cv::Point>> contoursToDraw;
    for (const auto& smoothStroke : smoothContours2f) {
    ...
    for (const auto& stroke : smoothContours2f) {
