...
int main(int argc, char **argv)
{
    ...
    cv::Mat lpf;
    cv::GaussianBlur(img, lpf, cv::Size(21, 21), 3);  // kernel 21×21, σ=3
    
    // Blur and compute high‑pass component
    cv::Mat blurred;
    cv::GaussianBlur(img, blurred, cv::Size(21, 21), 3);

    cv::Mat hpf;
    cv::subtract(img, blurred, hpf);                  // img – blurred
    cv::add(hpf, cv::Scalar(127, 127, 127), hpf);     // +127 to every channel

    cv::Mat binary;
    cv::threshold(hpf, binary, 128, 255, cv::THRESH_BINARY | cv::THRESH_OTSU); // used to be cv::THRESH_BINARY_INV
