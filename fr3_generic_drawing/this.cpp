...
double size = config["size"].as<double>(); // as a percentage
double pen_elevation = config["pen_elevation"].as<double>(); // in meters
double paper_width = config["paper_width"].as<double>(); // along y, in meters
double paper_length = config["paper_height"].as<double>(); // along x, in meters
double center_x = config["image_center_x"].as<double>(); // in meters
double center_y = config["image_center_y"].as<double>(); // in meters

const std::string IMAGE_PATH = "/home/qpaig/my_ros2_ws/src/fr3_generic_drawing/config/images" + image_file;
double CONVERSION_FACTOR;
const double DRAWING_HEIGHT = pen_elevation * std::pow(pen_elevation, 2.0);
...
int main(int argc, char **argv)
{
    ...
    const double image_width_px = img.cols;
